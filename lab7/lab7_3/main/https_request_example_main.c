...
#define WEB_PORT "443"
/*
#define WEB_URL "https://wttr.in/Santa+Cruz?format=3"
#define WEB_PATH "/Santa+Cruz?format=3"
*/
...
static void https_get_request(esp_tls_cfg_t cfg, const char *WEB_SERVER_URL, const char *REQUEST, char* buf)
{
    //char buf[512];
    int ret, len;
    esp_tls_t *tls = esp_tls_init();
    if (!tls) {
        ESP_LOGE(TAG3, "Failed to allocate esp_tls handle!");
        goto exit;
    }
    ...
    if (esp_tls_conn_http_new_sync(WEB_SERVER_URL, &cfg, tls) == 1) {
        ...
        if (ret == ESP_OK) {
            ESP_LOGE(TAG3, "TLS error = -0x%x, TLS flags = -0x%x", esp_tls_code, esp_tls_flags);
        }
        goto cleanup;
    }
    ...
    do {
        len = 512 - 1;
        //memset(buf, 0x00, 512);
        ret = esp_tls_conn_read(tls, (char *)buf, len); //reads in about 512ish bytes but it doesn't really matter since our weather data is small
...
{
    ESP_LOGI(TAG3, "https_request using crt bundle");
    esp_tls_cfg_t cfg = {
...
static void https_get_request_using_cacert_buf(char* buf, char* WEB_URL, char* HOWSMYSSL_REQUEST)
{
    ...
    https_get_request(cfg, WEB_URL, HOWSMYSSL_REQUEST, buf);
}
...
static void https_get_request_using_specified_ciphersuites(void)
{
    ...
    ESP_LOGI(TAG3, "https_request using server supported ciphersuites");
    esp_tls_cfg_t cfg = {
        .cacert_buf = (const unsigned char *) server_root_cert_pem_start,
        .cacert_bytes = server_root_cert_pem_end - server_root_cert_pem_start,
    ...
    ESP_LOGI(TAG3, "https_request using server unsupported ciphersuites");
...
static void https_get_request_using_global_ca_store(void)
{
    ...
    ESP_LOGI(TAG3, "https_request using global ca_store");
    esp_ret = esp_tls_set_global_ca_store(server_root_cert_pem_start, server_root_cert_pem_end - server_root_cert_pem_start);
    if (esp_ret != ESP_OK) {
        ESP_LOGE(TAG3, "Error in setting the global ca store: [%02X] (%s),could not complete the https_request using global_ca_store", esp_ret, esp_err_to_name(esp_ret));
        return;
    }
    esp_tls_cfg_t cfg = {
...
{
    ESP_LOGI(TAG3, "https_request to local server");
    esp_tls_cfg_t cfg = {
...
static void https_request_task(void *location, void *pvparameters)
{
    //printf("location: %s\n", (char *)location);
    char WEB_URL[100]= "https://";
    char WEB_PATH[100]= "/";
    strcat(WEB_PATH, location);
    strcat(WEB_PATH, "?format=3");
    
    strcat(WEB_URL, WEB_SERVER);
    strcat(WEB_URL, WEB_PATH);
    //printf("WEB_URL: %s\n", WEB_URL);
    char HOWSMYSSL_REQUEST[300] = "GET ";
    strcat(HOWSMYSSL_REQUEST, WEB_PATH);
    strcat(HOWSMYSSL_REQUEST, " HTTP/1.1\r\nHost: " WEB_SERVER);
    //strcat(HOWSMYSSL_REQUEST, WEB_SERVER);
    strcat(HOWSMYSSL_REQUEST, "\r\nUser-Agent: esp-idf/1.0 esp32\r\n\r\n");
    //printf("HOWSMYSSL_REQUEST: %s\n", HOWSMYSSL_REQUEST);
    ESP_LOGI(TAG3, "Starting https_request");
    ...
    if (strcmp(CONFIG_EXAMPLE_LOCAL_SERVER_URL, "FROM_STDIN") == 0) {
        ...
        ESP_LOGE(TAG3, "Configuration mismatch: invalid url for local server");
        abort();
    }
...
#endif
    */
    //ESP_LOGI(TAG3, "Minimum free heap size: %" PRIu32 " bytes", esp_get_minimum_free_heap_size());
    https_get_request_using_cacert_buf(pvparameters, WEB_URL, HOWSMYSSL_REQUEST);
    //https_get_request_using_global_ca_store();
    //https_get_request_using_specified_ciphersuites();
    //ESP_LOGI(TAG3, "Finish https_request example");
}
...
void app_main(void)
{
    ...
    if (esp_reset_reason() == ESP_RST_POWERON) {
        ESP_LOGI(TAG3, "Updating time from NVS");
