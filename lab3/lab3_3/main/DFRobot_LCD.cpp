 ...
 * @version  V1.0
 * @date  2017-2-10
 edited by Isabella Phung for CSE121 to be compatible with esp32c3
 */
...
#define LCD_SENSOR_ADDR (0x7c>>1) //LCD address
...
#define I2C_MASTER_FREQ_HZ   100000 //max for 1602 is 400kHz      /*!< I2C master clock frequency */
...
void DFRobot_LCD::clear()
{
    ...
    vTaskDelay(2 / portTICK_PERIOD_MS); // Wait 2 ms
}
...
void DFRobot_LCD::begin(uint8_t cols, uint8_t lines, uint8_t dotsize) 
{
    ...
    vTaskDelay(50 / portTICK_PERIOD_MS); // Wait 5 ms
...
void DFRobot_LCD::send(uint8_t *data, uint8_t len)
{
    ...
    */
    ...
    i2c_master_write_byte(cmd, (LCD_SENSOR_ADDR << 1) | I2C_MASTER_WRITE, true); //addresses the lcd
    for(int i = 0; i < len; i++){
        i2c_master_write_byte(cmd, data[i], true); //writes data
    }
    ...
    vTaskDelay(pdMS_TO_TICKS(50)); // Delay 1 seconds
}
...
void DFRobot_LCD::setReg(uint8_t addr, uint8_t data)
{
    ...
    */
    ...
    i2c_master_start(cmd); //begin to craft command
    i2c_master_write_byte(cmd, (LCD_SENSOR_ADDR << 1) | I2C_MASTER_WRITE, true); //addresses the lcd
    i2c_master_write_byte(cmd, data, true); //writes data
    i2c_master_stop(cmd); //done writing command
    ret = i2c_master_cmd_begin(I2C_MASTER_NUM, cmd, 1000 / portTICK_PERIOD_MS); //send command
    i2c_cmd_link_delete(cmd);
