...
void accel_read(int* distance) {
    ...
    if (ret == ESP_OK) {
        ...
        //ESP_LOGI("Accelerometer", "values: ax=%d, ay=%d", ax, ay);
        if(ax > 50 && ax < 500){
            distance[0] = 1; 
            ESP_LOGI("Accelerometer", "LEFT ");
        } else if (ax < -50 && ax > -500){
            distance[0] = -1; 
            ESP_LOGI("Accelerometer", "RIGHT ");
        } else if (ax > 500 && ax < 5500 && ax != 511 && ax != 1023){
            distance[0] = 5; 
            ESP_LOGI("Accelerometer", "FAR LEFT ");
        } else if (ax < -500 && ax > -5500){
            distance[0] = -5; 
            ESP_LOGI("Accelerometer", "FAR RIGHT ");
        } else {
            distance[0] = 0;
        }
        ...
        } else if (ay < -50 && ay > -500){
            distance[1] = 1; 
            ESP_LOGI("Accelerometer", "UP ");
        } else if (ay > 500 && ay < 5500){
            distance[1] = -5; 
            ESP_LOGI("Accelerometer", "FAR DOWN ");
        } else if (ay < -500 && ay > -5500){
            distance[1] = 5; 
            ESP_LOGI("Accelerometer", "FAR UP ");
        } else {
            distance[1] = 0;
        }
    }
